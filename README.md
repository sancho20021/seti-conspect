...
```
Чтобы создать несколько соединений (например, для множества клиентов сайта), в сокете используется опция `SO_REUSEADDR`.
...
3. Бывает так, что на одном IP-адресе располагается несколько сайтов, и хочется блокировать лишь некоторые из них. Для этого необходимо лезть внутри IP-пакета. HTTPs шифрует все, кроме названия сайта. Поэтому блокировщик может профильтровать пакеты по вхождению названия.
  ...
  - Чтобы это обойти, можно использовать фрагментацию IP-пакетов, чтобы разрезать имя сайта и положить в два пакета.
...
- Нагнать энтропии в префикс домена (GoOgLe.com вместо google.com), чтобы злоумышленнику нужно было еще и угадать раскладку каждого символа.
...
**Как проверить подлинность корневого сервера?**

Корневой публичный ключ самоподписан. Но злоумышленник может сказать, что он тоже является корневым сервером.

Чтобы удостовериться в подлинности корневого сервера, в операционной системе зашиты публичные ключи всех корневых серверов.
